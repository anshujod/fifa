...
def render() -> None:
    ...
    def _colour_row(row):
        styles = [""] * len(row)
        if row["Better"] == team1:
            styles[1] = "background-color:rgba(52,211,153,.10); font-weight:600"
        elif row["Better"] == team2:
            styles[2] = "background-color:rgba(52,211,153,.10); font-weight:600"
        return styles
...
"""
    ...
    if total == 0:
        ...
        # Dominance bar — reuse the shared outcome-bar component (brand-consistent)
        t1_pct = h2h["team1_wins"] / total
        t2_pct = h2h["team2_wins"] / total
        d_pct  = h2h["draws"] / total
        theme.outcome_bar(t1_pct, d_pct, t2_pct, team1, team2)
