...
def _form_letters(wins: int, draws: int, losses: int) -> str:
    ...
    colours = {"W": "#10B981", "D": "#94A3B8", "L": "#EF4444"}
    spans = "".join(
        f"<span style='color:{colours[lt]};font-weight:600;margin-right:8px'>{lt}</span>"
        for lt in letters[:5]
    )
    return f"<div style='font-size:14px;letter-spacing:.02em'>{spans}</div>"
...
def render() -> None:
    theme.page_header(
        ...
        subtitle=(
            "Select any two World Cup 2026 teams to estimate win probabilities, "
            "expected goals, and the simulated scoreline distribution."
        ),
    )
    ...
    venue = "Neutral venue" if neutral else "Home advantage applied"

    bh, bc, ba = st.columns([2, 1, 2])
    with bh:
        st.markdown(f"### {flag(home)} {home}")
        if len(elo_h):
            st.caption(f"Elo {int(elo_h[0])}")
    with bc:
        st.markdown(
            "<div style='text-align:center;margin-top:6px;font-size:13px;"
            "font-weight:600;letter-spacing:.08em;color:#94A3B8'>VS</div>"
            f"<div style='text-align:center;font-size:11.5px;color:#64748B;"
            f"margin-top:4px'>{venue}</div>",
            unsafe_allow_html=True,
        )
    with ba:
        st.markdown(
            f"<h3 style='text-align:right;margin-bottom:0'>{flag(away)} {away}</h3>",
            unsafe_allow_html=True,
        )
        if len(elo_a):
            st.markdown(
                f"<div style='text-align:right;font-size:13px;color:#94A3B8'>"
                f"Elo {int(elo_a[0])}</div>",
                unsafe_allow_html=True,
            )
    ...
    eg1.metric(f"Expected goals — {home}", f"{lam_h:.2f}")
    eg2.metric(f"Expected goals — {away}", f"{lam_a:.2f}")

    # ── Scoreline distribution ────────────────────────────────────────────────
    theme.section(
        "Scoreline Distribution",
        f"Probability of each scoreline across {n_sims:,} simulated matches.",
    )
    ...
    with ch2:
        ...
        st.markdown(
            ...
            "text-transform:uppercase;color:#94A3B8;margin:14px 0 10px'>"
